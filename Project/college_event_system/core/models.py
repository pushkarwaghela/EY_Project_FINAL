...
class AttendanceRecord(models.Model):
    ...
    def status(self):
        ...
        if not self.event or not self.marked_at:
            return 'unknown'
        
        event_start = datetime.combine(self.event.date, self.event.start_time)
        if self.marked_at > event_start + timedelta(minutes=15):
            return 'late'
        return 'on_time'
