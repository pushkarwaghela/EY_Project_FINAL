...
def admin_generate_qr(request, event_id):
    try:
        ...
        print(f"Error generating QR: {e}")  # For debugging
        return JsonResponse({
            'success': False,
            'message': str(e)
        }, status=500)
...
@login_required
@user_passes_test(is_admin)
def admin_generate_qr_page(request):
    """Display the QR code generation page"""
    # Get upcoming events - FIXED: Using correct field name
    events = Event.objects.filter(
        date__gte=timezone.now().date()
    ).order_by('date', 'start_time')  # Changed from 'time' to 'start_time'
...
# View to generate QR code via AJAX (POST request)
@require_POST
@csrf_exempt  # Remove this in production after testing
def generate_qr_code(request, event_id=None):
    """Generate QR code for an event"""
    try:
        # Get event_id from URL parameter or request body
        if event_id is None:
            data = json.loads(request.body)
        ...
        # Create QR code data - FIXED: Using correct field names
        qr_data = {
            'event_id': event.id,
            'event_code': event.event_id,  # Changed from event_code to event_id if that's your field
            'title': event.title,
            'date': str(event.date),
            'start_time': str(event.start_time) if event.start_time else '',
            'end_time': str(event.end_time) if event.end_time else '',
            'type': 'attendance',
            'url': f'http://127.0.0.1:8000/attendance/scan/{event.event_id}/'  # Adjust if needed
        }
        ...
        # FIXED: Using correct field names for event info
        event_info = {
            'id': event.id,
            'title': event.title,
        ...
        if event.start_time:
            event_info['start_time'] = event.start_time.strftime('%H:%M')
        if event.end_time:
            event_info['end_time'] = event.end_time.strftime('%H:%M')
        
        # Add event code/ID
        event_info['code'] = event.event_id  # Or use event.event_code if that field exists
...
def some_view(request):
    ...
    print(f"View function: {request.resolver_match.func.__name__}")